...
import { FiPlus, FiEdit2, FiHome } from 'react-icons/fi';
...
/* Barra de ações – fica acima do grid */
    ...
    <Container>
      ...
      <Grid>
        {hospedagens.map((item) => {
          ...
          return (
            <Card key={id}>
              {imagem && <Banner src={imagem} alt={titulo} />}
              <CardBody>
                <h2>{titulo}</h2>
                <p>
                ...
                {nome_caracteristica.length > 0 && (
                  <>
                    <strong>Características:</strong>
                    <Tags>
                      {nome_caracteristica.map((c, i) => (
                        <Tag key={i}>{c}</Tag>
                      ))}
                    </Tags>
                  </>
